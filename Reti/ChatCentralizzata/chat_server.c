...
void processRegister(RequestArgs* args) {
    ...
    if (userExists(args->ctx->list, rx.sender_nick)) {
        ...
        // Se è un utente nuovo lo registriamo fisicamente su file e in memoria
        FILE* fd = fopen(FILENAME, "a+");
        if (fd) {
...
void processLookupProxy(RequestArgs* args) {
    ...
    if (destPort != -1) {
        // Trovato il destinatario: giriamo il pacchetto esattamente come è arrivato (Proxy)
        struct sockaddr_in targetAddress = createAddress(LOCALHOST, destPort);
        Packet forwardPack = rx; 
        packetToNetworkByteOrder(&forwardPack); 
        ...
        // L'utente non esiste o è offline
