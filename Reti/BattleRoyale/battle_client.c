...
void* handleTcpConnection(void* args) {
    ...
    while (1) {
        ...
        if (rx_pack.type == TYPE_SAFE_ZONE) {
            // Sottraiamo la vita in modo sicuro
            pthread_mutex_lock(ctx->life_mutex);
            ctx->life -= rx_pack.health_left; // health_left qui funge da "danno subito"
    ...
    // Segnaliamo al thread UDP di fermarsi mettendogli la vita a 0
    pthread_mutex_lock(ctx->life_mutex);
    ctx->life = 0;
    pthread_mutex_unlock(ctx->life_mutex);
...
void* handleUdpConnection(void* args) {
    ...
    while (1) {
        ...
        if (current_life <= 0) {
            break; // Esci dal thread se siamo morti o disconnessi
        }

        // Simulazione movimento
        int posx = 10 + rand() % 41; // 10-50
        int posy = 10 + rand() % 41;
        
        // Possibile danno ambientale casuale
        if (rand() % 10 > 7) { 
            pthread_mutex_lock(ctx->life_mutex);
            ctx->life -= (rand() % 5);
        ...
        sleep(1); // Manda telemetria ogni secondo
    }
...
int main(int argc, char* argv[]) {
    ...
    // Aspettiamo che il giocatore muoia (thread TCP si chiude)
