...
void* handleTcpConnection(void* args){
    ...
    if (connect(par.sockTcp, (struct sockaddr*) &par.serverAddressTcp, sizeof(par.serverAddressTcp)) < 0){
        perror("[-] Errore di connessione al server dell'ospedale");
        exit(EXIT_FAILURE);
    }
    ...
    // Aspettiamo la conferma dal server
    ...
    // Restiamo in ascolto per eventuali allerte (Codice Rosso)
    while(1){
        MedPacket packNew;
        if (recv(par.sockTcp, &packNew, sizeof(packNew), 0) > 0) {
            packetToHostByteOrder(&packNew);
            if (packNew.type == TYPE_CODE_RED){
                printf("\n[URG] CODICE ROSSO RICEVUTO! Rianimazione in arrivo. Disconnessione.\n");
                close(par.sockTcp);
                close(par.sockUdp);
                exit(EXIT_SUCCESS);
            }
        } else {
            printf("[-] Connessione con l'ospedale persa.\n");
            break;
        }
    }
    return NULL;
...
int main(int argc, char* argv[]){
    if (argc < 4) {
        fprintf(stderr, "Uso: %s <patient_id> <client_port_TCP> <client_port_UDP>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    pthread_create(&threadUdp, NULL, handleUdpConnection, &par); 

    // Aspettiamo solo il TCP: se muore lui, il client si chiude
