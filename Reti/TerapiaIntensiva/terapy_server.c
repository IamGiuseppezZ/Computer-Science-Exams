...
void* handleThreadTcpClient(void* args) {
    ...
    while(1) {
        ...
        if (bytes_received > 0) {
            ...
            printf("[-] Patient disconnected (Socket: %d).\n", ctx.client_socket);
...
void* handleTcpConnection(void* args) {
    ...
    if (listen(ctx.server_socket, 10) < 0) {
        perror("[-] Error listening server TCP");
        exit(EXIT_FAILURE);
    }

    printf("[SERVER] TCP Hospital unit listening for patient admissions...\n");
...
void* handleUdpConnection(void* args) {
    ...
    printf("[SERVER] UDP Vitals monitoring active...\n");
            ...
            if (receivedPacket.type == TYPE_VITALS_UPDATE) {
                ...
                if (receivedPacket.bpm < 40 || receivedPacket.oxygen < 90) {
                    ...
                    if (sockTcpClient != -1) {
                        ...
                        printf("[!] DISPATCHED CODE RED TO PATIENT %d\n", receivedPacket.patient_id);
                    }
                }
            }
        }
