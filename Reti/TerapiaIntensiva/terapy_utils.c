...
void printPacket(const MedPacket* packet) {
    ...
    switch (packet->type) {
        case TYPE_PATIENT_ADMIT:
            printf(" [▶] TIPO: RICOVERO PAZIENTE\n");
            printf("----------------------------------------\n");
            printf(" > ID Paziente : %d\n", packet->patient_id);
            break;

        case TYPE_ADMIT_ACK:
            printf(" [✓] TIPO: CONFERMA RICOVERO\n");
            printf("----------------------------------------\n");
            printf(" > ID Paziente : %d\n", packet->patient_id);
            break;

        case TYPE_VITALS_UPDATE:
            printf(" [♥] TIPO: AGGIORNAMENTO VITALI\n");
            printf("----------------------------------------\n");
            printf(" > ID Paziente : %d\n", packet->patient_id);
            printf(" > BPM         : %d\n", packet->bpm);
            printf(" > SpO2        : %d%%\n", packet->oxygen);
        ...
        case TYPE_CODE_RED:
            printf(" [🚨] TIPO: CODICE ROSSO (CRITICO!)\n");
            printf("----------------------------------------\n");
            printf(" > ID Paziente : %d\n", packet->patient_id);
            printf(" > AZIONE      : RIANIMAZIONE IMMEDIATA\n");
            break;
